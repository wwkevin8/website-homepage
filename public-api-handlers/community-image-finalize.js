...
const { finalizeCommunityImage } = require("../api/_lib/community");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    try {
      const image = await finalizeCommunityImage(supabase, user, body);
      created(res, { image });
    } catch (error) {
      sendError(res, error, "Image finalize failed.");
    }
  } catch (error) {
