(function () {
  ...
  function renderUsersTable(payload) {
          ...
          <tbody>
            ${items.map(item => `
              <tr>
                ...
                <td><span class="admin-status-badge ${getBadgeClass("provider", item.last_login_provider)}">${AdminShell.escapeHtml(item.last_login_provider || "Google")}</span></td>
                <td>${Number(item.login_count || 0)}</td>
                <td><span class="admin-table-note">\u8be6\u60c5\u80fd\u529b\u9884\u7559</span></td>
              </tr>
            `).join("")}
          </tbody>
        </table>
    ...
    if (pagination) {
      ...
      pagination.textContent = `\u7b2c ${meta.page || 1} / ${meta.total_pages || 1} \u9875\uff0c\u5171 ${meta.total || 0} \u6761`;
    }
  }
          ...
          <tbody>
            ${items.map(item => `
              <tr>
                ...
                <td><span class="admin-status-badge ${getBadgeClass("manager-status", item.status)}">${getManagerStatusLabel(item.status)}</span></td>
                ...
                <td>
                  <div class="admin-table-actions">
                    ...
                    ${item.status === "active"
                      ? `<button class="button button-text is-danger" type="button" data-manager-disable="${item.id}" ${currentSession?.admin?.id === item.id ? "disabled title=\"\u5f53\u524d\u8d26\u53f7\u4e0d\u80fd\u505c\u7528\u81ea\u5df1\"" : ""}>\u505c\u7528</button>`
                      : `<button class="button button-text" type="button" data-manager-enable="${item.id}">\u542f\u7528</button>`}
                  </div>
                </td>
              </tr>
            `).join("")}
  ...
  async function initManagersPage() {
    ...
    table?.addEventListener("click", event => {
      ...
      const enableId = target.getAttribute("data-manager-enable");
      if (enableId) {
        openConfirmModal({
          title: "\u786e\u8ba4\u542f\u7528\u7ba1\u7406\u5458",
          text: "\u542f\u7528\u540e\uff0c\u8be5\u7ba1\u7406\u5458\u53ef\u91cd\u65b0\u767b\u5f55\u540e\u53f0\u5e76\u6309\u89d2\u8272\u8303\u56f4\u4f7f\u7528\u7cfb\u7edf\u3002",
          confirmText: "\u786e\u8ba4\u542f\u7528",
          onConfirm: async () => {
            try {
              await AdminApi.enableManager(enableId);
              showMessage("\u542f\u7528\u6210\u529f");
              await render();
            } catch (error) {
              showMessage(error.message, true);
      ...
      const disableId = target.getAttribute("data-manager-disable");
      if (disableId) {
        openConfirmModal({
          title: "\u786e\u8ba4\u505c\u7528\u7ba1\u7406\u5458",
          text: "\u505c\u7528\u540e\uff0c\u8be5\u8d26\u53f7\u5c06\u65e0\u6cd5\u7ee7\u7eed\u767b\u5f55\u540e\u53f0\u3002",
          confirmText: "\u786e\u8ba4\u505c\u7528",
          danger: true,
          onConfirm: async () => {
            try {
              await AdminApi.disableManager(disableId);
              showMessage("\u505c\u7528\u6210\u529f");
              await render();
            } catch (error) {
              showMessage(error.message, true);
