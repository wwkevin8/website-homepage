...
| `/api/public/transport-groups` | public aggregate -> `transport-groups.js` | GET | Public/public-safe transport group listing; full groups are filtered out of public list responses | No | No | `transport_groups_public_view`, `transport_group_members`, `transport_requests` |
...
| `/api/transport-groups` | `api/transport-groups.js` -> `api/transport-groups/index.js` | GET, POST | Admin list/create groups; GET enriches P6A dispatch workbench rows with member summaries, luggage totals, payment/offline-recorded summaries, public visibility, and readonly risk flags; top-level shim keeps Vercel from serving `api/transport-groups/index.js` as static source on the extensionless route | Yes | Yes | `transport_groups`, `transport_groups_public_view`, `transport_requests`, `transport_group_members` |
| `/api/transport-groups/:id` | `api/transport-groups/[id].js` | GET, PATCH, DELETE | Admin group detail/update/delete; GET includes P6A dispatch/member details and readonly risk flags, while PATCH logs group field changes to `admin_operation_logs` | Yes | Yes | `transport_groups`, `transport_group_members`, `transport_requests`, `admin_operation_logs` |
...
| Resend | Transactional email | `auth-email.js`, `transport-order-submission-email.js`, `storage-order-notifier.js`, `transport-sync-audit-email.js`, `storage-sync-audit-email.js`, `membership-birthday-reminders.js` | `RESEND_API_KEY`, sender env vars | Some flows require Resend; production sender needs confirmation. Membership birthday reminders are sent to advisors/operators, not students. |
