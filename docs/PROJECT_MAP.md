...
| `/api/admin/membership-claims/:id/unbind-order` | `api/admin/[...action].js` | POST | For reserved storage claims, atomically clears both `storage_orders.membership_benefit_claim_id` and the claim reverse link, restores the claim to selected, preserves financial fields, and writes membership/admin audit logs. Transport claims remain blocked from this legacy entry. | Yes | Yes | `storage_orders`, `membership_benefit_claims`, `storage_membership_unbind_operations`, `membership_audit_logs`, `admin_operation_logs` |
