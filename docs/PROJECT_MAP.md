...
| `/api/admin/storage-orders-export` | `api/admin/[...action].js` | GET | Export the current storage-order filter result; `order_type=all` uses the operator execution-view columns and applies the same expanded-row service/date/charge/payment filters as the Vue workbench | Yes | Yes | `storage_orders`, `site_users` |
