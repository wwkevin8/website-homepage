...
| `/api/public/transport-board` | public aggregate -> `transport-board.js` | GET | Public transport board data; default public results include only current/future shareable groups, sort by `flight_datetime` ASC for upcoming views, filter full groups out of public list responses, and exclude `source = admin_manual` P4a supplement orders from the public board | No | No | `transport_requests`, `transport_groups`, `transport_group_members` |
| `/api/public/transport-groups` | public aggregate -> `transport-groups.js` | GET | Public/public-safe transport group listing; default results include only current/future timed groups, upcoming sort is by effective service time ASC, full groups are filtered out, and internal `dispatch_status` is stripped before response | No | No | `transport_groups_public_view`, `transport_group_members`, `transport_requests` |
