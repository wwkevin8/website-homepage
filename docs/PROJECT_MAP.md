...
| `/api/admin/storage-orders` | `api/admin/[...action].js` | GET, PATCH, DELETE | Storage order list/detail/update/delete via query `id`; `order_type=all` returns the Vue storage workbench list expanded across ST-B/ST-P/ST-S and supports service type, charge status, payment status, offline-recorded, operator, date, sort, and keyword filters. List responses include `current_stats`; detail responses include recent `operation_logs`. Bulk PATCH only sets/cancels `offline_recorded` for selected ids | Yes | Yes | `storage_orders`, `site_users`, `admin_operation_logs` |
