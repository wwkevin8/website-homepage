...
| `pickup-form.html` | Pickup/dropoff or transport request form | 用户前台 | `site-auth.js`, `script.js`, `pickup-form.js`, `styles.css` | `/api/public/transport-request-submit`, `/api/public/my-transport-requests`, `/api/auth/session`, `/api/public/membership/me` | Auth-related user form; uses the required arrival/departure datetime as the public service-time input and does not collect or submit `preferred_time` or the former cosmetic `deadline_date`; shows a read-only NGN pickup membership hint when the current live claim is pickup selected/reserved. |
...
| `postage.html` | Postage service public information page and static estimator | 前台 | `site-auth.js`, `script.js`, `postage-data.js`, `postage.js`, `styles.css` | None | Static V1 service page for UK/China/domestic postage, certificate UPS, milk powder, packaging, prohibited-item notices, tracking links, and contact CTA. No order, payment, database, email, API, or admin integration. |
| `postage-submit.html` | Logged-in postage request submission form | 用户前台 | `site-auth.js`, `postage-submit.js`, `styles.css` | `/api/public/postage-order-submit`, `/api/auth/session` | Requires existing user login, creates `POST-YYYYMMDD-###` postage requests, separates box-delivery and pickup dates, blocks unavailable box 2, and sends only the student confirmation email with `/img/storage-service-qr.jpg`; no customer-service reminder email, payment, logistics sync, or estimator snapshot binding. |
...
| `/admin/postage/orders` | Postage work-order management | 后台 Vue | `apps/admin-vue` | `/api/admin/postage-orders` | Admin-only postage workbench with list filters, status/box-delivery/payment filters, search, copy summary, and right-side drawer editing; no jump-style detail page and no customer-service email reminder. |
...
| `/api/public/membership/me` | public aggregate -> `membership-me.js` | GET | Current user's membership entitlement/claim status for current cycle; storage claims linked to a storage order use that order's latest membership discount/final-price fields for display so historical claim snapshots do not show stale free amounts | Yes | No | `membership_entitlements`, `membership_benefit_claims`, `storage_orders` |
...
| `/api/public/postage-order-submit` | public aggregate -> `postage-order-submit.js` | POST | Submit logged-in postage request and send the student confirmation email with客服微信、电话和 `/img/storage-service-qr.jpg`; customer-service reminder email is intentionally not sent | Yes, via user session | No | `postage_orders`, `postage_order_logs`, `site_users`, `postage_order_counters` |
...
| `/api/public/transport-request-submit` | public aggregate -> `transport-request-submit.js` | POST | Submit pickup/dropoff/transport request using `flight_datetime` as the public service time; `preferred_time_start/end` remain nullable legacy columns and are not required or sent by the public form | Yes | No | `transport_requests`, `site_users`, `order_number_counters`, possibly `orders` via DB trigger |
...
| `/api/public/transport-board` | public aggregate -> `transport-board.js` | GET | Public transport board/detail data; supports `limit`, `page`, filters, and service-time sort aliases where `upcoming/service_time_asc/nearest/asc` are nearest-to-farthest and `farthest/service_time_desc/desc` are farthest-to-nearest; default public results include only current/future shareable groups, nearest upcoming service time first, filter full groups out of public list responses, exclude `source = admin_manual` P4a supplement orders, and runs the server empty-group cleanup for groups with zero effective active members after the 10-minute grace window before listing | No | No | `transport_requests`, `transport_groups`, `transport_group_members` |
| `/api/public/transport-groups` | public aggregate -> `transport-groups.js` | GET | Public/public-safe transport group listing; supports `limit`, `page`, status/date/group filters, and service-time sort aliases where `upcoming/service_time_asc/nearest/asc` are nearest-to-farthest and `farthest/service_time_desc/desc` are farthest-to-nearest; default results include only current/future effective groups, nearest upcoming service time first, DB-level pagination, no member detail payload, full groups filtered out, internal `dispatch_status` stripped before response, and server empty-group cleanup before listing | No | No | `transport_groups_public_view`, `transport_group_members`, `transport_requests` |
| `/api/public/transport-join-preview` | public aggregate -> `transport-join-preview.js` | POST | Preview pickup/dropoff Group joining from Group status, frontend visibility, service time and actual `max_passengers`; effective counts exclude Request `closed/cancelled`, and member `shareable` does not control Group access | Yes | No | `transport_requests`, `transport_groups`, `transport_group_members` |
| `/api/public/transport-join-submit` | public aggregate -> `transport-join-submit.js` | POST | Submit a pickup/dropoff Group join through the service-role-only atomic/idempotent RPC. A stable `submission_id` safely replays one submission intent; business-state conflicts return coded 409 and malformed/mismatched input returns 400 | Yes | No | `transport_requests`, `transport_groups`, `transport_group_members` |
...
| `/api/admin/memberships` | `api/admin/[...action].js` | GET, POST | List memberships with claims, audit logs, latest operation summary, activation-code source, advisor/user display fields, and manually grant membership entitlements. GET combines search, cycle, status, benefit type, and `advisor_admin_id` filtering before exact totals and pagination; advisor resolution strictly falls back through assigned advisor, creator, granter, then activation-code generator, while `unassigned` matches only an empty fallback chain. | Yes | Yes | `membership_entitlements`, `membership_benefit_claims`, `site_users`, `admin_users`, `membership_activation_codes`, `membership_audit_logs` |
| `/api/admin/memberships?view=advisors` | `api/admin/[...action].js` | GET | Return active administrators plus inactive administrators still resolved as a historical membership advisor; the single-level route is compatible with the deployed Vercel aggregate function | Yes | Yes | `membership_entitlements`, `membership_activation_codes`, `admin_users` |
...
| `/api/admin/membership-claims/:id/unbind-order` | `api/admin/membership-claims/[claimId]/unbind-order.js` -> `api/_lib/storage-membership-unbind.js` (the aggregate catch-all delegates to the same handler) | POST | Explicit Vercel multi-segment entry for reserved storage claims. Atomically clears both `storage_orders.membership_benefit_claim_id` and the claim reverse link, restores the claim to selected, preserves financial fields, and writes membership/admin audit logs. Transport/manual/unknown claims are rejected by the shared handler. | Yes | Yes | `storage_orders`, `membership_benefit_claims`, `storage_membership_unbind_operations`, `membership_audit_logs`, `admin_operation_logs` |
...
| `/api/admin/storage-orders` | `api/admin/[...action].js` | GET, PATCH, DELETE | Storage order list/detail/update/delete via query `id`; `order_type=all` returns the Vue storage workbench list expanded across ST-B/ST-P/ST-S and supports service type, charge status, payment status, offline-recorded, operator, date, sort, and keyword filters. List responses include `current_stats`; detail responses include recent `operation_logs`. Pricing recalculation on membership-linked storage orders also syncs the linked membership claim discount/final-price snapshot. Bulk PATCH only sets/cancels `offline_recorded` for selected ids | Yes | Yes | `storage_orders`, `site_users`, `membership_benefit_claims`, `admin_operation_logs` |
| `/api/admin/postage-orders` | `api/admin/[...action].js` | GET, PATCH | Admin-only postage work-order list/detail/update via query `id`; supports order status, independent box-delivery status, payment status, quick filters, search, pagination, and right-drawer editing. It does not send customer-service reminder emails. | Yes | Yes | `postage_orders`, `postage_order_logs`, `site_users` |
| `/api/admin/storage-orders-export` | `api/admin/[...action].js` | GET | Export the current storage-order filter result; `order_type=all` uses the operator execution-view columns, omits pinyin from the execution export, keeps time slot after the address column, and applies the same expanded-row service/date/charge/payment filters as the Vue workbench | Yes | Yes | `storage_orders`, `site_users` |
...
| `/api/transport-manual-import/membership` | `api/transport-manual-import/membership.js` | POST | Admin-only atomic membership pickup manual entry; validates the session administrator, member/claim/advisor ownership, duplicate warnings and group capacity, then creates the request/group membership and reserves the claim in one transaction with idempotency. | Yes | Yes | `transport_requests`, `transport_groups`, `transport_group_members`, `membership_benefit_claims`, `transport_membership_manual_operations`, audit logs |
| `/api/transport-membership-members` | `api/transport-membership-members.js` | GET | Authenticated admin member search and available pickup entitlement context with masked identity fields. | Yes | Yes | `site_users`, `membership_entitlements`, `membership_benefit_claims`, `admin_users` |
| `/api/transport-requests/:id/membership` | `api/transport-requests/[id]/membership.js` | GET, POST | Read current pickup membership relationship or atomically link, replace, and unlink an existing request through the dedicated service-role RPC. | Yes | Yes | `transport_requests`, `membership_benefit_claims`, `membership_entitlements`, `transport_membership_admin_operations`, audit logs |
...
| `/api/transport-requests/export` | `api/transport-requests/export.js` | GET | Admin request export; selected IDs remain explicit, while current-filter export uses the same membership projection and shared membership/advisor filters as the list before the existing export limit | Yes | Yes | `admin_transport_requests_membership_view` |
| `/api/transport-groups` | `api/transport-groups.js` -> `api/transport-groups/index.js` | GET, POST | Admin list/create groups; GET supports `paginate`, `page`, `page_size`, `validity`, service-time `sort` aliases where `service_time_asc/nearest/asc` are nearest-to-farthest and `service_time_desc/farthest/desc` are farthest-to-nearest, group/date/status/visibility filters, and keyword matching across group id plus member request fields; before listing it deletes groups older than 10 minutes with zero effective active members while preserving original `transport_requests`; default paginated list is active/effective groups, nearest upcoming service time first, 20 rows per page, then enriches only the page with P6A dispatch workbench summaries, risk flags, and internal P6B-B2 `dispatch_status` from the base `transport_groups` table; top-level shim keeps Vercel from serving `api/transport-groups/index.js` as static source on the extensionless route | Yes | Yes | `transport_groups`, `transport_groups_public_view`, `transport_requests`, `transport_group_members` |
| `/api/transport-groups/:id` | `api/transport-groups/[id].js` | GET, PATCH, DELETE | Admin group detail/update/delete; GET includes P6A dispatch/member details, readonly risk flags, internal `dispatch_status`, driver name/phone/note fields where available, and existing group/member `admin_operation_logs`; PATCH can update `dispatch_status` separately from lifecycle `status` and writes `dispatch_status_update`; P6 Recovery removed the driver email notification POST action | Yes | Yes | `transport_groups`, `transport_group_members`, `transport_requests`, `admin_operation_logs` |
...
| `supabase/20260530_postage_orders.sql` | Creates postage request tables and order number allocation | Adds `postage_orders`, `postage_order_logs`, independent postage order/box-delivery/payment statuses, service-role-only RLS posture, and `POST-YYYYMMDD-###` allocation; apply before using `/postage/submit` or `/admin/postage/orders`. |
...
| `supabase/migrations/20260828164011_transport_join_atomic_idempotency.sql` | Atomic public Group join and capacity enforcement | Adds nullable submission identity/hash fields, a partial unique index, a service-role-only atomic join RPC, and Member INSERT/UPDATE capacity trigger. The RPC and JS evaluator share the Group allowlist `single_member` / `active` / `open`; `draft` / `full` / `closed` / `cancelled` remain blocked. No historical rows are rewritten. |
| `supabase/migrations/20260828213000_transport_public_group_active_member_counts.sql` | Aligns public Group aggregate counts | Preserves the Production 23-column contract (`id = transport_groups.id` UUID first, `group_id` text second), owner/grants/options intent and public data boundary while excluding Members whose Request is `closed/cancelled` from member, passenger and luggage aggregates. Preview and Production share Supabase, so this database migration must be applied and verified before deploying the dependent API/frontend. |
...
| `supabase/20260525_transport_group_empty_cleanup.sql` | P6 Recovery no-op note | Do not apply for the current release; empty groups are deleted by server code after a 10-minute grace window and a fresh zero-effective-active-member check, with no `last_empty_at` schema requirement. |
...
| `supabase/migrations/20260824190000_admin_transport_requests_membership_view.sql` | Admin-only transport membership/advisor projection | Read-only `security_invoker` and `security_barrier` view; resolves direct or unique exact reverse claims, strictly resolves the entitlement advisor, revokes public access, and grants SELECT only to `service_role`. |
...
| `postage_orders` | Postage request work orders | `/api/public/postage-order-submit`, `/api/admin/postage-orders`, postage email helper | Yes | Independent postage V1 table. Main `status` tracks only the overall postage flow (`new` through `cancelled`); `box_delivery_status` separately tracks `not_required/pending/arranged/delivered/issue`. Student confirmation email status is stored here; there is no customer-service reminder email status. |
| `postage_order_logs` | Postage admin operation logs | `/api/admin/postage-orders` | Yes | Service-role/admin-only log table for create/update/status/fee/payment/logistics/internal-note changes. |
...
| `transport_groups_public_view` | Public/admin group listing view | public groups, admin groups, stats helpers | Public-safe intended | Production contract is 23 columns with first `id` UUID from `transport_groups.id` and second `group_id` text; P6B-B2 keeps `dispatch_status` out of this shared view, and admin group APIs merge the internal field from `transport_groups` directly. |
...
| Field | Values | Notes |
| --- | --- | --- |
| `status` | `new`, `contacted`, `pending_pickup`, `picked_up`, `weighed_pending_quote`, `pending_payment`, `paid`, `shipped`, `completed`, `cancelled` | Main postage workflow only. Box delivery states must not be mixed into this field. |
| `box_delivery_status` | `not_required`, `pending`, `arranged`, `delivered`, `issue` | Independent box-delivery workflow; admin filters can combine this with `status`. |
| `payment_status` | `not_required`, `pending`, `paid`, `refunded`, `cancelled` | Admin-only payment tracking; V1 does not implement online payment. |
| `customer_email_status` | `pending`, `sent`, `failed`, `skipped` | Student confirmation email status. Email failure does not roll back order creation. |
...
| `api/_lib/postage-order-notifier.js` | Postage student confirmation email | Resend only; includes客服微信、电话和 `/img/storage-service-qr.jpg` absolute QR image URL. Does not send customer-service reminder emails. |
...
| Resend | Transactional email | `auth-email.js`, `transport-order-submission-email.js`, `storage-order-notifier.js`, `postage-order-notifier.js`, `transport-sync-audit-email.js`, `storage-sync-audit-email.js`, `membership-birthday-reminders.js` | `RESEND_API_KEY`, sender env vars | Some flows require Resend; production sender needs confirmation. Postage V1 sends only the student confirmation email and intentionally has no internal customer-service reminder email. |
...
| `SITE_URL` | Site URL fallback for storage/postage emails | May appear in emails | No/low | Yes if storage or postage emails active | Optional. |
| `VERCEL_URL` | Vercel deployment URL fallback | May appear in emails | No/low | Usually automatic | Used as fallback by storage and postage notifiers. |
...
| `POSTAGE_EMAIL_FROM` | Sender for postage student confirmation emails | May appear in emails | No/medium | Yes if postage emails active | Optional override; falls back to auth/SMTP sender and then a safe default. This is not a customer-service notification recipient. |
