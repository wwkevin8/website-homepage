...
| `/api/public/transport-board` | public aggregate -> `transport-board.js` | GET | Public transport board data; full groups are filtered out of public list responses | No | No | `transport_requests`, `transport_groups`, `transport_group_members` |
...
| `/api/transport-requests` | `api/transport-requests/index.js` | GET, POST, PATCH | Admin list/create transport requests; GET supports offline-recorded, import batch/source, dynamic last-operator filters, and Vue customer-service workbench fields; PATCH supports selected-row offline-recorded updates and writes operation logs | Yes | Yes | `transport_requests`, `site_users`, `transport_group_members`, `admin_operation_logs` |
...
| `/api/transport-manual-import/preview` | `api/transport-manual-import/preview.js` | POST | Admin-only manual/bulk import preview; maps legacy sheet fields, normalizes airport/terminal/service/date fields including UK-style `DD/MM/YYYY`, checks duplicates and group warnings, and does not write business rows | Yes | Yes | `transport_requests`, `transport_groups`, `transport_group_members` |
| `/api/transport-manual-import/commit` | `api/transport-manual-import/commit.js` | POST | Admin-only bulk import commit; reruns server-side mapping/cleaning/validation, creates `transport_requests`, creates or joins groups, records `import_batch_id`, and logs operations | Yes | Yes | `transport_requests`, `transport_groups`, `transport_group_members`, `admin_operation_logs` |
| `/api/transport-manual-import/manual` | `api/transport-manual-import/manual.js` | POST | Admin-only single manual supplement; uses the same server-side import service and requires explicit confirmation for warning joins | Yes | Yes | `transport_requests`, `transport_groups`, `transport_group_members`, `admin_operation_logs` |
...
| `/api/transport-requests/export` | `api/transport-requests/export.js` | GET | Admin request export with offline-operation columns; supports selected IDs, current page IDs, or all current filters | Yes | Yes | `transport_requests`, `site_users` |
