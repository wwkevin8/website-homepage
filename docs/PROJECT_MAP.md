...
| `/admin-vue/storage/sync-logs` | Storage sync audit logs | 后台 Vue | `apps/admin-vue` | `/api/storage-sync-audit-logs`, `/api/run-storage-sync-audit` | Read-only storage sync audit logs with manual admin run; scheduled passive cron and daily summary email use the cron-only route. |
...
| `/api/transport-requests` | `api/transport-requests/index.js` | GET, POST, PATCH | Admin list/create transport requests; GET supports offline-recorded and dynamic last-operator filters; PATCH supports bulk marking the current filtered result as offline-recorded | Yes | Yes | `transport_requests`, `site_users`, `transport_group_members` |
| `/api/transport-requests/:id` | `api/transport-requests/[id].js` | GET, PATCH, DELETE | Admin read/update/delete request; GET includes transport request operation logs; PATCH records `last_operated_by`, `last_operated_at`, and changed fields in `admin_operation_logs`; DELETE removes the request from any transport group and syncs affected group state before deleting the request | Yes | Yes | `transport_requests`, `transport_group_members`, `site_users`, `admin_operation_logs` |
...
| `transport_requests` | Pickup/dropoff/carpool requests | public submit/my/board/join, admin transport APIs, cron, order sync SQL, membership helper | Yes | Public board must expose safe subset only. Membership pickup benefit only applies to `service_type=pickup`, not dropoff. Admin-only operational fields now include `offline_recorded`, `last_operated_by`, and `last_operated_at`. |
