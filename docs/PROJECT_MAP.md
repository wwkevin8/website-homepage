...
| `/api/public/transport-board` | public aggregate -> `transport-board.js` | GET | Public transport board/detail data; supports `limit`, `page`, filters, and `sort=upcoming/service_time_desc`; default public results include only current/future shareable groups, nearest upcoming service time first, filter full groups out of public list responses, exclude `source = admin_manual` P4a supplement orders, and avoid maintenance writes during public GET | No | No | `transport_requests`, `transport_groups`, `transport_group_members` |
| `/api/public/transport-groups` | public aggregate -> `transport-groups.js` | GET | Public/public-safe transport group listing; supports `limit`, `page`, status/date/group filters, and `sort=upcoming/service_time_desc`; default results include only current/future effective groups, nearest upcoming service time first, DB-level pagination, no member detail payload, full groups filtered out, and internal `dispatch_status` stripped before response | No | No | `transport_groups_public_view`, `transport_group_members`, `transport_requests` |
