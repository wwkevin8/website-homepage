...
| `/api/admin/memberships/advisors` | `api/admin/[...action].js` | GET | Return active administrators plus inactive administrators still resolved as a historical membership advisor; used by the transport membership ownership filter | Yes | Yes | `membership_entitlements`, `membership_activation_codes`, `admin_users` |
