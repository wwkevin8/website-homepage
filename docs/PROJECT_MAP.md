...
| `/api/transport-groups` | `api/transport-groups.js` -> `api/transport-groups/index.js` | GET, POST | Admin list/create groups; GET supports `paginate=true&page=...&page_size=...` for the carpool group admin first screen, with default UI using active validity and service-time ascending order; `mode=list` returns a lightweight first-screen page without cleanup, duplicate-future checks, risk construction, payment/offline enrichment, or dispatch-readiness enrichment, while still returning stable pagination metadata from the same filtered query (`total`, `total_pages`, `has_next`, `has_prev`); `enrich_only=true&ids=...` enriches the current page group ids asynchronously for payment/offline summaries, readonly risk flags, and internal P6B-B2 `dispatch_status`; server filters cover service type, airport, group status, validity, service date range, frontend visibility, and keyword/group-member search; top-level shim keeps Vercel from serving `api/transport-groups/index.js` as static source on the extensionless route | Yes | Yes | `transport_groups`, `transport_groups_public_view`, `transport_requests`, `transport_group_members` |
