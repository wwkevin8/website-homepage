...
| `/api/public/transport-groups` | public aggregate -> `transport-groups.js` | GET | Public/public-safe transport group listing; supports lightweight `mode=preview&limit=10` for the homepage and server-side `page/pageSize` pagination plus `group_id`, service type, airport, and date filters for the full board. Uses page-size-plus-one for `has_next` instead of exact count by default and excludes internal notes/payment/log/dispatch fields. | No | No | `transport_groups_public_view`, `transport_group_members`, `transport_requests` |
...
| `supabase/20260526_public_transport_groups_perf_indexes.sql` | Public transport group listing indexes | Adds pg_trgm plus public-board indexes for visible joinable groups, service/airport/time filters, group-id search, and member summary lookup. |
