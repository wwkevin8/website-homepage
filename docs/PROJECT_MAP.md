...
| `/api/admin/memberships` | `api/admin/[...action].js` | GET, POST | List memberships with claims/audit logs and manually grant membership entitlements | Yes | Yes | `membership_entitlements`, `membership_benefit_claims`, `site_users`, `membership_audit_logs` |
| `/api/admin/memberships/:id` | `api/admin/[...action].js` | DELETE | Delete a membership entitlement and its linked benefit claim records for the current admin workflow | Yes | Yes | `membership_entitlements`, `membership_benefit_claims`, `membership_audit_logs` |
...
| `/api/admin/membership-codes` | `api/admin/[...action].js` | GET, POST | List one-time membership activation codes or generate a new code shown once | Yes | Yes | `membership_activation_codes`, `membership_audit_logs` |
...
| `community_post_images` | Community post image metadata | `/api/public/community-image-upload`, `/api/public/community-image-finalize`, `/api/public/community-posts`, community helper | Yes | Images are allowed only on `second_hand` and `sublet`, up to 3 active images per post. Storage bucket is private; public detail responses return short-lived signed URLs only and do not expose storage paths or original filenames. |
