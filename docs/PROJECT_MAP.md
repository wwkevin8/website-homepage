...
| `admin-memberships.html` | NGN membership entitlement admin | 后台 | `admin-api.js`, `admin-shell.js`, `admin-memberships.js`, `styles.css` | `/api/admin/memberships`, `/api/admin/memberships/users`, `/api/admin/membership-claims/*` | First-stage manual grant; user search by name/email/phone/WeChat; entitlement/claim list with linked order and discount fields; mark-used, cancel/reset, and audit log only. |
