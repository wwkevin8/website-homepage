...
| `/api/public/transport-groups` | public aggregate -> `transport-groups.js` | GET | Public/public-safe transport group listing; full groups are filtered out of public list responses, internal `dispatch_status` is stripped before response, and rows include a public-safe `target_request_id` for the first active group member so the join modal can target an existing order without filtering away the full board. | No | No | `transport_groups_public_view`, `transport_group_members`, `transport_requests` |
