...
| `/api/public/transport-groups` | public aggregate -> `transport-groups.js` | GET | Public/public-safe transport group listing; full/closed groups are filtered out of public list responses, internal `dispatch_status` is stripped before response, rows include a public-safe `target_request_id` for the first active group member, and the endpoint now applies future-date filtering, nearest-first sorting, server-side pagination, and `mode=preview` limit handling before member enrichment. | No | No | `transport_groups_public_view`, `transport_group_members`, `transport_requests` |
...
| `supabase/migrations/20260526150111_users_source_user_composite_unique.sql` | Registration user sync fix | Adds/keeps `users(source_user_table, source_user_id)` uniqueness and replaces `sync_user_from_site_users()` so registration can sync `site_users` into `users` without `ON CONFLICT` or null `users.id` failures. |
