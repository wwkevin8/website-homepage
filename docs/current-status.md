...
- Preview: Fix commit `58eb4025594a3ab0f0cd430ecdac49c33d5e0451` was pushed only to `origin/release-membership-transport-20260824` and deployed as Vercel preview `dpl_D1LYYn27vsJjK3aLLgT2ZEJj3Gz1` (`https://webside-loqlwgokf-wwkevin8s-projects.vercel.app`). The build is READY. A Vercel-authenticated request to `/api/admin/memberships?view=advisors` returned the application JSON 401 and no-store headers instead of platform `404_NOT_FOUND`, proving the single-level route reaches the authenticated Serverless handler. The preview browser domain had no admin application session, so authenticated advisor contents were not re-read from production; that behavior remains covered by the local real-HTTP/local-Supabase integration test.
- Open risks / follow-up: Re-deploy the final documentation commit to preview so the tested deployment maps to the final candidate HEAD. Do not redeploy production until the user accepts the preview evidence or supplies a safe preview admin login session for an additional authenticated browser check.
