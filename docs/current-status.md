...
- Verification: Syntax checks, focused regression guards, full regression check, local mocked request submission, required production preflight, and read-only live acceptance are required for this release. No production write test is permitted.
- Open risks / follow-up: Historical notes containing `截止日期` remain unchanged by design. No automatic join deadline was introduced.
