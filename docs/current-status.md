...
- Release candidate: Pricing QA commits `6eeef0ed8b5fa9ca6daf58383c677ae6799c91b6`, `85ec155c457ddcc379cf76b96bc4f95f3bf7a060` and `2a0d987` are fast-forwarded to both `origin/release/transport-storage-preflight` and `origin/release/membership-transport-admin-20260831`. Preview `dpl_6dtrY6yt77A2k1UsrnS93Fe9h9Bb` is READY at `https://webside-kdgfsuyfj-wwkevin8s-projects.vercel.app` and includes the accepted application source plus the protected-Preview QA improvements.
- Verification: The focused local matrix passed September £105 to October £100 repricing, ordinary/member combinations, paid/unpaid invariance, passenger tiers, closed-member exclusion and cross-terminal surcharge. The real Preview browser flow passed create, join, regroup, date repricing (£105→£100), capacity, remove, close, delete and recreate across personal center, public board and administrator detail. On the same Preview, membership security/link/used-correction, ordinary and membership manual entry, storage atomic unlink, public membership full chain, Chinese audit copy, desktop and 390px tests passed. Migration ledger is 24/24 aligned with zero pending; transport relationship conflicts and all scoped QA residue are zero.
- Open risks / follow-up: No pricing release blocker remains. Create the final documentation-only handoff commit, verify its Preview provenance, then Production may be deployed only under the explicit production release procedure. Existing dependency audit findings are unchanged because this work changed no dependencies.

- Date: 2026-08-31
- Scope: Added the narrowly scoped Vercel P0 route fix for the already-migrated storage membership atomic unlink flow in the isolated release worktree. Production application deployment remains pending Preview verification; the three membership migrations are already applied and must not be replayed.
- Summary: The previously catch-all-only multi-segment endpoint returned platform `404 NOT_FOUND` on Vercel. A new explicit `api/admin/membership-claims/[claimId]/unbind-order.js` deployment entry delegates to the same shared `api/_lib/storage-membership-unbind.js` handler used by the aggregate admin route. The shared handler owns method, cookie/role, UUID, reason, expected-state, RPC, response and error-redaction behavior; the explicit route contains no copied business logic.
- Open risks / follow-up: Build-route inspection and a fresh protected Preview must prove the endpoint reaches application authentication before Production application deployment. The main dirty worktree is out of scope and must remain untouched.

- Date: 2026-08-28
- Scope: Prepared and locally verified the isolated P0 release branch `codex/p0-transport-join-release-20260828` from `origin/release/transport-storage-preflight` for public transport Group join authorization, capacity, atomicity and idempotency. Commit `75dd872a0d73056f3b89e57205552b1c553d7ea3` is superseded by the follow-up open-state alignment in the same branch and must not be used as the final release commit. Its GitHub push automatically created three Vercel Preview deployments; all were non-Production, and the Production deployment and custom domains did not change. Production migrations remain unapplied and Production still has the original defect.
- Summary: Grouped public-board rows and Preview/Submit derive joinability from Group status, frontend visibility, service time and real `max_passengers`; no member Request `shareable` value can hide or close a public open Group, while standalone private Requests remain filtered. The JS evaluator and service-role-only atomic RPC now share the exact allowed Group statuses `single_member`, `active` and `open`; `draft`, `full`, `closed` and `cancelled` remain blocked. A stable frontend `submission_id` serializes same-user and final-seat races, creates Request+Member in one transaction, and safely replays a completed submission. Effective counts consistently exclude Request `closed/cancelled`; business-state rejection is coded HTTP 409 while malformed/mismatched input remains 400.
- Verification: Focused P0 and full regression tests passed. After temporarily aligning only the confirmed loopback `transport_groups_status_check` with the Production seven-state contract, real Preview/Submit tests proved `open` pickup and dropoff joins, same-submission concurrent 201/200 behavior, later idempotent 200 replay, and a two-account final-seat 201/409 result. The original local five-state constraint was then restored exactly; its validation/deferrability attributes and the RPC, capacity Trigger and public-view definitions remained unchanged. The wider integration suite covered capacities 4/5/6, same/different submission concurrency, response-loss replay, two/three/five-account final-seat races, Group state matrix, inactive-member counts, Member INSERT/UPDATE/DELETE/cross-Group movement, public RPC competing with backend/direct/update paths, and forced Member failure rollback. No deadlock, over-capacity state, duplicate membership, orphan public-join Request or success/failure mismatch remained. Exact cleanup left zero QA rows and temporary functions/triggers; six protected non-QA empty Groups were unchanged.
- Open risks / follow-up: The release remains **NO-GO** until a recoverable Production backup/PITR restore point and recovery authority are confirmed. The two migrations remain unapplied, so Production still has the original Group-shareability/capacity/concurrency defects and must not be described as fixed. Preview and Production share Supabase; Preview transport Submit must not be used before the compatible atomic RPC/capacity migration and UUID-compatible 23-column public-view migration are applied and verified in Production. Real public board/Group GET, browser board-to-modal E2E, Production RLS/permission behavior, Production performance and a standard migration-complete rebuild remain unverified.

- Date: 2026-08-25
- Scope: Prepared an isolated release candidate from production release `2f1edd0a83312366f65c3fcba40ce0df97762a2a` for the admin Membership Benefits advisor filter. Changes are limited to the membership list query/UI, focused tests, minimal documentation, and rebuilt admin assets; no public page, membership mutation flow, database structure/data, email, production migration, push, or deployment was changed.
- Summary: The Membership Benefits filter bar now adds a dynamic “所属顾问” selector using the deployed authenticated endpoint `/api/admin/memberships?view=advisors`, with all, unassigned, active administrators, and inactive administrators that remain historically associated. The backend resolves exactly one effective advisor using `advisor_admin_id → created_by_admin_id → granted_by_admin_id → activation-code generated_by_admin_id`; unassigned requires all four sources to resolve empty. Search, cycle, membership status, benefit type, claim/display status, and advisor filtering now run against one backend collection before exact totals and page slicing, and an out-of-range requested page clamps to the last valid page (or page 1 for no results).
- Verification: Focused advisor priority/combination/pagination assertions passed, including a simulated PostgREST server cap below the requested batch size. A real local-Supabase scale fixture exercised 1,000, 1,001, 2,500, and 5,000 entitlements through the authenticated admin handler. All totals/pages/last pages were exact, broad search returned 999/1,000/2,499/4,999 matches rather than stopping at 100, post-1000 specific-advisor/unassigned/pickup-used matches were present, and no duplicate IDs occurred. At 2,500 rows the full five-condition combination took 66.92ms/9 requests and heavy advisor/benefit filters took 60.67–62.12ms/8 requests; at 5,000 rows they took 119.49ms/11 requests and 111.75–114.96ms/11 requests. Maximum measured heap growth was about 7.4MB and maximum URL length 4,162 characters. The fixture cleanup restored users/entitlements/claims to the exact pre-test counts and ID hashes. Full regression, API/test syntax, `git diff --check`, and the production Vue build passed; the build generated `index-A4ELlbfr.js` plus `index-DfT5owXD.css` with only the existing large-chunk warning. The real built CSS also passed 1440px one-row and 390px no-overflow layout checks.
- Open risks / follow-up: The new candidate is not yet committed, pushed, previewed, or deployed. Entitlements and search matches use exact-count 1,000-row pages with offsets advanced by the actual returned row count, so a lower PostgREST row cap cannot silently truncate the result. Large result sets read claims only for the selected membership cycle in the same manner; small result sets use one ID-bounded lookup. Activation-code IDs remain deduplicated and capped at 100 per request. The 5,000-row local result is acceptable for the current backend workflow, but response time and memory should still be monitored as cycle membership grows beyond this tested range. Dependency audit continues to report the existing package vulnerabilities (root: 2 high; admin: 1 low and 3 high), with no dependency changes in this task.
...
- Preview: Fix commit `58eb4025594a3ab0f0cd430ecdac49c33d5e0451` was pushed only to `origin/release-membership-transport-20260824` and deployed as READY preview `dpl_D1LYYn27vsJjK3aLLgT2ZEJj3Gz1`. After recording that evidence, documentation commit `cb4470b67a8198b843c96ec0686a64d4cd794e4e` was pushed to the same temporary branch and deployed as READY preview `dpl_F6N4euXn7WxtDCyRKdpAHKSXB8pk` (`https://webside-1yjtxotkb-wwkevin8s-projects.vercel.app`). On both previews, a Vercel-authenticated request to `/api/admin/memberships?view=advisors` returned the application JSON 401 and no-store headers instead of platform `404_NOT_FOUND`, proving the single-level route reaches the authenticated Serverless handler. The preview browser domain had no admin application session, so authenticated advisor contents were not re-read from production; that behavior remains covered by the local real-HTTP/local-Supabase integration test.
- Open risks / follow-up: Do not redeploy production until the user accepts the preview evidence or supplies a safe preview admin login session for an additional authenticated browser check. The formal release branch remains on `d870c760f7348b59ae31614b40186a96c407019a`; only the temporary remote branch contains this fix.
...
- Verification: Local PostgreSQL transaction and persistent-local migration checks passed with 55 view rows / 55 distinct order IDs and permissions `anon=false`, `authenticated=false`, `service_role=true`. The 12-order transaction fixture covered strict fallback, direct/reverse/ambiguous/unlinked, multiple cycles, live transfer and disabled advisors, then rolled back to 55 unchanged orders. Authenticated API/browser/export coverage returned linked=10, unlinked=2, advisor A=4 and combined review=2; pagination, overflow clamp, list/export identity, dynamic active/inactive advisors, 1440px desktop and 390px mobile no-overflow checks passed. The real public activation→entitlement→pickup claim→transport request→bidirectional binding→view→admin list/export chain passed and cleaned its fixture. Syntax, regression, Vite build and diff checks passed.
- Open risks / follow-up: Apply the view migration before deploying code. Dependency audits still report the release branch's existing findings (root 2 high; admin 1 low and 3 high); no dependency changed. Production migration and deployment remain pending explicit release continuation.

- Date: 2026-08-24
- Scope: Released the public pickup/dropoff request and join-carpool submission-state fix only. The release changes the two public submit flows, shared browser API response classification, focused public styles, regression guards, and an API-intercepted Playwright test. No backend handler, database, matching, email, admin behavior, deployment configuration, production data, or unrelated dirty-worktree change was included.
- Summary: Duplicate-order checking and formal creation now show distinct loading messages and sanitized per-stage durations. Summaries render only after an HTTP 200/201 response with a valid `orderNo`; confirmed success immediately locks the button as `已提交`, explicit failure preserves values and restores retry, and uncertain responses lock submission while offering copy-verification and support actions. The old pre-submit/failure summary and locally generated fallback registration number were removed.
- Release: Feature commit `010a29c856e30bc9a86e8f903c9cb370dc933338` (`fix: improve transport submission states`) was pushed to `origin/release/transport-storage-preflight`. Vercel production deployment `dpl_vWBzny9wTjsewWXLRBschYtpymr6` is `READY` at `https://webside-9f5kr1fil-wwkevin8s-projects.vercel.app` and aliased to `https://ngn.best` and `https://www.ngn.best`.
- Verification: The required admin Vue build, `node --check api/admin/[...action].js`, and `node scripts/regression-check.js` passed in a clean worktree. The focused Playwright test passed against a static local server with every API call intercepted, so no database or real order was touched. Post-deployment read-only checks returned HTTP 200 for the pickup form, transport board, admin entry, relevant public scripts, and stylesheet; live source markers confirmed the hidden initial summary, distinct duplicate-check/create copy, uncertain-result handling, and explicit 200/201 join acceptance. No production form was submitted.
- Open risks / follow-up: Existing dependency audits report pre-existing high-severity findings; this release did not change dependencies or lockfiles. Persistent centralized performance telemetry remains out of scope; current timing diagnostics are browser-console objects containing only event, flow, stage, outcome, and duration.

- Date: 2026-08-24
- Scope: Removed only the ineffective public carpool `deadline_date` input/display from the request form and join modal. Updated the two public frontend scripts, focused Playwright selector coverage, regression guards, project map, and this status note. No admin UI, API, database schema/history, `preferred_time_end`, email, pricing, matching, group lifecycle, deployment config, or unrelated worktree changes are included.
- Summary: Public request submission no longer renders or validates a carpool deadline, includes it in confirmation summaries, or appends `截止日期` text to new `transport_requests.notes`. The join modal and success summary no longer show the derived read-only date. Join preview and submit payload serialization remain unchanged because `deadline_date` was never serialized there. Existing `flight_datetime` / `flight_time_reference` expiry, visibility, sorting, and joinability behavior remains active.
- Verification: `npm.cmd --prefix apps/admin-vue run build`, `node --check api/admin/[...action].js`, syntax checks for the four affected JavaScript/test files, and `node scripts/regression-check.js` passed. Local mocked submission confirmed no deadline control/key/note/summary and preserved `flight_datetime`. Read-only production checks returned HTTP 200 for the request form, transport board, public groups API, and public script; live source contains no deadline reference, retains the flight field/join summary logic, and the public groups API returns active data. No production form submission, account creation, or order creation was performed.
- Release: Functional/docs commit `bead04774e61248a017631d3c243827e645019e5` was pushed to `origin/release/transport-storage-preflight` before production deployment. Vercel production deployment `dpl_6PgQmfeXbrT4tYn1cqKvRwUT7uEZ` is `READY`, URL `https://webside-dtbbzlc7t-wwkevin8s-projects.vercel.app`, aliased to `https://ngn.best`.
- Open risks / follow-up: Historical notes containing `截止日期` remain unchanged by design. No automatic join deadline was introduced. Dependency installation reported existing audit findings (root: 2 high; admin: 1 low and 3 high), not changed in this release.

- Date: 2026-08-24
- Scope: Removed the public carpool preferred pickup-time field from both the pickup/dropoff request form and the join-carpool modal. Updated only the public form/display scripts, public transport submit/list handlers, shared carpool join/group lifecycle timing behavior, focused regression/Playwright coverage, project map, and this status note. No admin UI, database schema or historical rows, deployment configuration, Git commit/push, or production deployment was changed.
- Summary: New public requests and joins no longer render, validate, summarize, serialize, send, or synthesize `preferred_time_start/end`. Join preview/submit matching, date comparison, time-gap warnings, public group sorting, and public displayed service time now use actual `flight_datetime` and group `flight_time_reference`. Existing nullable database columns and historical values remain intact for compatibility and admin/history reads. The transport Playwright flow was also made safe for localhost by forcing `LOCAL_SUPABASE_*` and updated to stop using removed/stale form selectors.
- Verification: Syntax checks passed for the affected frontend, API, lifecycle, regression, and Playwright files. `node scripts/regression-check.js` passed. `PLAYWRIGHT_BASE_URL=http://localhost:3000 npm run qa:playwright:transport-flow` passed against local Supabase and covered request submission, join preview/submit, public board/group synchronization, regrouping, closure, deletion, and recreation. Source searches found no `preferred_time` or preferred-time label in the two public form scripts or their public submit/join handlers. GitHub commit `f6190d9ab550725fb6733cbef4d8deb2fd1da4d6` was pushed to `origin/release/transport-storage-preflight`; Vercel production deployment `ALvrFN7VYFRQHbCj28qK9k7Cm3eW` completed successfully and is current on `https://ngn.best`. Post-deploy read-only checks confirmed the live initiate form and join bundle contain no preferred-time field/reference while retaining `flight_datetime` and the existing UK-time notice.
- Open risks / follow-up: The first transport-flow attempt exposed a pre-existing safety bug: before the localhost override was added, the script created five QA-only `site_users` rows in the default configured Supabase project (`qa_transport_pw_flow_1787582815419_u1@example.com` through `_u5@example.com`) and then stopped before creating any transport orders. On 2026-08-24, an explicit read-only audit confirmed all five accounts had zero transport, storage, postage, membership, community, login-event, or password-reset associations; the five exact QA accounts were then deleted by UUID and a post-delete lookup confirmed none remained. The localhost override now prevents recurrence.

- Date: 2026-07-08
- Scope: Released a lightweight public pickup/carpool submit loading interaction. This touched only `pickup-form.js`, `transport-public.js`, `styles.css`, and this status note. No database schema, API route, backend/admin behavior, email behavior, submit payload shape, or deployment config was changed.
...
- Release: GitHub commit `0648ca0` (`fix: add loading state for public pickup submissions`) was pushed to `origin/release/transport-storage-preflight`. Vercel production deployment `dpl_Ey1QnLUo5AieSj4Lu4u5ZXCbd6BM` is `READY` at `https://webside-6qn4p6m6f-wwkevin8s-projects.vercel.app`, with aliases `https://ngn.best` and `https://www.ngn.best`.
- Verification: `node --check pickup-form.js`, `node --check transport-public.js`, `node --check api/admin/[...action].js`, `node scripts/regression-check.js`, and `npm.cmd --prefix apps/admin-vue run build` passed. Production Vercel inspect confirmed project `webside`, target `production`, status `Ready`, and aliases `https://ngn.best` / `https://www.ngn.best`. Production no-write browser verification on `https://ngn.best` used mocked public API responses to avoid creating test orders and confirmed: desktop and mobile pickup submit shows "提交中..." with the car loader, is disabled, sets `aria-busy`, and rapid repeat clicks send one submit; the summary keeps `2026/09/18 19:30` and `2026/09/18 21:00` and does not show `12:30`; join-carpool preview shows "检查中..." and blocks repeat checks; join submit shows "加入中..." and blocks repeat submits; success renders the existing success result; failure restores the submit button to an enabled "确认加入拼车" state; mobile-width pages had no horizontal overflow in the checked flows.
- Open risks / follow-up: No production write-through was performed; submit and join APIs were intercepted during verification to avoid test orders. A human mobile screenshot spot check can still confirm the exact visual feel of the new inline car icon on the customer's device.

- Date: 2026-07-08
- Scope: Fixed the public frontend transport submission summary timezone conversion bug. This release changes `pickup-form.js`, `transport-public.js`, `scripts/regression-check.js`, and this status note. `pickup-form.html` was reviewed and left at the current release version because the UK-time warning is already present there. No database schema, API save logic, admin display, existing order data, email template/function, deployment config, or production data was changed.
- Summary: The customer-reported wrong "拼车表单摘要" was confirmed to come from `pickup-form.js` `buildSummary()`, whose flight and preferred time lines previously used `formatDateTime()` and therefore ran raw `datetime-local` values through `new Date(...)` plus `timeZone: "Europe/London"`. Public submission summaries now format raw `datetime-local` text directly, so `2026-09-18T19:30` displays as `2026/09/18 19:30` and `2026-09-18T21:00` displays as `2026/09/18 21:00` without timezone conversion. The public join-carpool success summary in `transport-public.js` received the same raw-text summary treatment.
- Release: GitHub commit `442a58b` (`fix: prevent public pickup summaries from timezone-shifting local times`) was pushed to `origin/release/transport-storage-preflight`. Vercel production deployment `dpl_3qxQ1Ycgh8RGRwVeHfoQwdP1NqMP` is `READY` at `https://webside-5ektavxnd-wwkevin8s-projects.vercel.app`, with aliases `https://ngn.best` and `https://www.ngn.best`.
- Verification: `node --check pickup-form.js`, `node --check transport-public.js`, `node --check api/admin/[...action].js`, `node scripts/regression-check.js`, and `npm.cmd --prefix apps/admin-vue run build` passed. Production Vercel inspect confirmed project `webside`, target `production`, status `Ready`, and aliases `https://ngn.best` / `https://www.ngn.best`. Production no-write browser verification on `https://ngn.best/pickup-form.html` confirmed `2026-09-18T19:30` displays as `2026/09/18 19:30`, `2026-09-18T21:00` displays as `2026/09/18 21:00`, and `12:30` / `14:00` do not appear in the generated public summary.
- Open risks / follow-up: `api/_lib/transport-order-submission-email.js` still formats saved server timestamps with `Europe/London`, but it was intentionally left unchanged because the screenshot text is not generated by that email function and the saved/admin display path is currently correct. If the product later requires emails to echo the exact pre-save browser input text, that should be handled as a separate API/data-contract change. No production order was created during verification.
- Date: 2026-07-08
...
- Release: GitHub commit `81c3b67` (`fix: add UK time notice to join carpool modal`) was pushed to `origin/release/transport-storage-preflight`. Vercel production deployment `dpl_77ExYsxQTUiqZED1o8TzNDdePnKS` is `READY` at `https://webside-69ox74hff-wwkevin8s-projects.vercel.app`, with aliases `https://ngn.best` and `https://www.ngn.best`.
- Verification: Static inspection confirmed the warning was added to both join form templates and the existing time field names remain unchanged. `node --check transport-public.js`, `node --check api/admin/[...action].js`, `node scripts/regression-check.js`, and `npm.cmd --prefix apps/admin-vue run build` passed. Production Vercel inspect confirmed project `webside`, target `production`, status `Ready`, and aliases `https://ngn.best` / `https://www.ngn.best`. Production GET verification confirmed `https://ngn.best/transport-public.js` contains the join modal UK-time notice.
- Open risks / follow-up: No known release blocker for this display-only change. A human modal screenshot check can still confirm exact placement after login because automated verification did not submit or create production data.
...
- Verification: Static inspection confirmed the change is limited to the existing `.carpool-time-hint` paragraph style; the warning text, time field names, input types, submit payload, and validation logic remain unchanged. `node --check pickup-form.js`, `node --check api/admin/[...action].js`, `node scripts/regression-check.js`, and `npm.cmd --prefix apps/admin-vue run build` passed. Production Vercel inspect confirmed project `webside`, target `production`, status `Ready`, and aliases `https://ngn.best` / `https://www.ngn.best`. Production GET verification confirmed `https://ngn.best/pickup-form.html` contains the rainbow gradient style and right-shift margin for the UK-time notice.
- Open risks / follow-up: No known release blocker for this visual-only change. A human mobile screenshot spot check can still confirm the exact rainbow rendering and wrap on the customer's device.
...
- Verification: Static inspection confirmed the change is limited to the existing `.carpool-time-hint` paragraph style and text position; the time field names and input types remain unchanged. `node --check pickup-form.js`, `node --check api/admin/[...action].js`, `node scripts/regression-check.js`, and `npm.cmd --prefix apps/admin-vue run build` passed. Production Vercel inspect confirmed project `webside`, target `production`, status `Ready`, and aliases `https://ngn.best` / `https://www.ngn.best`. Production GET verification confirmed `https://ngn.best/pickup-form.html` contains the red style and spacing for the UK-time notice.
- Open risks / follow-up: No known release blocker for this visual-only change. A human mobile screenshot spot check can still confirm the exact wrap looks right on the customer's device.
