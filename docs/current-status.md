...
- Verification: `node --check pickup-form.js`, `node --check transport-public.js`, and `node scripts/regression-check.js` passed before commit. The regression check now guards the pickup summary and join summary against reintroducing shared/timezone formatting for raw `datetime-local` summary fields.
