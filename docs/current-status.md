...
- Date: 2026-08-28
- Scope: Prepared and fully verified an isolated, uncommitted P0 release candidate from `origin/release/transport-storage-preflight` for public transport Group join authorization, capacity, atomicity and idempotency. Changes are limited to the public board/join runtime, focused regression/integration diagnostics, two additive migrations, and minimal documentation. Approved Production access was limited to read-only schema metadata for the existing 23-column public-view contract; no Production business rows or API were read or written, and no migration or deployment was performed.
- Summary: Grouped public-board rows and Preview/Submit now derive joinability from Group status, frontend visibility, service time and real `max_passengers`; no member Request `shareable` value can hide or close a public open Group, while standalone private Requests remain filtered. A stable frontend `submission_id` and service-role-only atomic database RPC serialize same-user and final-seat races, create Request+Member in one transaction, and safely replay a completed submission. Effective counts consistently exclude Request `closed/cancelled`; business-state rejection is coded HTTP 409 while malformed/mismatched input remains 400.
- Verification: Focused P0 and full regression tests passed. Local loopback integration covered pickup/dropoff, capacities 4/5/6, same/different submission concurrency, response-loss replay, two/three/five-account final-seat races, Group state matrix, inactive-member counts, Member INSERT/UPDATE/DELETE/cross-Group movement, public RPC competing with backend/direct/update paths, and forced Member failure rollback. No deadlock, over-capacity state, duplicate membership, orphan public-join Request or success/failure mismatch remained. Exact cleanup left zero QA rows and temporary functions/triggers; six protected non-QA empty Groups were unchanged. One earlier effective-count assertion anomaly did not reproduce in three dedicated diagnostic rounds or subsequent suites.
- Open risks / follow-up: The two migrations remain unapplied to Preview/Production, so Production still has the original Group-shareability/capacity/concurrency defects and must not be described as fixed. Real public board/Group GET, browser board-to-modal E2E, Production RLS/permission behavior, Production performance and a standard migration-complete rebuild remain unverified. Request status is canonically `published/matched/closed`, so Request `cancelled` is defensive static coverage while Group `cancelled` is database-tested. Preview and Production share Supabase: release must first back up/confirm recovery, apply and verify the compatible atomic RPC/capacity migration, then apply and verify the UUID-compatible 23-column public-view migration, and only then deploy the RPC-dependent API/frontend.
