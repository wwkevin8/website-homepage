...
- Date: 2026-08-31
- Scope: Diagnosed the isolated release gate that compared a pre-repricing personal-center quote with a post-repricing public-board quote. No pricing formula, payment field, membership discount, database migration or production state was changed.
- Summary: `current_average_price_gbp` is the current live group quote per passenger and is computed by the shared `loadGroupStatsMap` / `computeTransportGroupPricingSnapshot` path for personal requests, the public board and administrator group consumers. The observed £105 was the two-passenger LHR peak-season quote before the test moved the group date from September into October; £100 was the correctly recomputed normal-season quote after that date change. The flow now refreshes both personal orders and administrator group detail after repricing, then compares those fresh results with the public board. A focused pricing matrix proves ordinary/member and paid/unpaid membership do not alter the public group quote or mutate individual financial fields.
- Verification: The focused matrix passed September £105 to October £100 repricing, ordinary/member combinations, paid/unpaid invariance, passenger tiers, closed-member exclusion and cross-terminal surcharge. The full isolated browser transport flow passed on the explicitly pinned local candidate at `http://localhost:4322`, including create, join, regroup, group-date repricing, remove, close, delete, recreate, personal center, public board and administrator detail. P0 final-seat/concurrency and regression checks passed with zero QA residue.
- Open risks / follow-up: Commit this test correction as a small release follow-up, deploy a fresh Preview from that exact commit, rerun the pricing and membership/storage release gates, then deploy Production only if the Preview and ledger/residue checks remain clean.
